...
function fig = createWindow()
	...
	freqTexts = gobjects(1, 3);
	%strings que os textos terao:
	freqTextStrings = {'Bass', 'Mid', 'Treble'};
	% strings dos ganhos:
	gainTexts = gobjects(1, 3);
	dBTexts = gobjects(1, 3);
	% esse for cria os 10 sliders do eq grafico
	for i=1:3
		sliders(i) = uicontrol( ...
			'Style', 'slider', ...
			'Parent', panel, ...
			'Position', [50+90*(i-1) 10 20 150], ...
		...
		freqTexts(i) = uicontrol( ...
			'Style', 'text', ...
			'Parent', fig, ...
			'Position', [panel.Position(1)+30+90*(i-1) 5 60 15], ...
...
function tocar(sinal, Fs, ganhos, playButton)
	...
	sinalEqualizado = controleTom3Botoes(Fs, sinal, ganhos);
